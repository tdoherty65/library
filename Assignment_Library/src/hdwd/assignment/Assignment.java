...
public class Assignment {
	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				System.out.println("4. Q3. Search For Book by ISBN - Almost working");
				System.out.println("5. Q4  Lend Book (Limited to 5 books) - Almost working");
				...
				// Q1a. insert book and author
					...
					System.out
							.println("The record has been successfully entered.");
				}
				// Q1b. add a borrower
				else if (option == 2) {
				...
				// Q2. Search for book based on author
					...
					for (int i = 0; i < books.length; i++) {
						System.out
								.printf("Book #%d : ID=%s, isbn=%s, title=%s, pubyear=%s \n",
										i, books[i].id, books[i].isbn,books[i].title, books[i].pubyear);
					}
				}

				// 3. Display how many copies of a Book in library (Display book
				// count by ISBN)
				else if (option == 4) {
				...
				// 4. Lend Book (Limited to 5 books)
					...
					int copyno = keyboard.nextInt();
					
					System.out.println("Enter Date dd/mm/yy.");
					String date = keyboard.nextLine();
					
					Borrower borrin = new Borrower(ssn, titleno, copyno, date);
					
					lendBook(conn, borrin);
				...
				// 5. BONUS display all authors BONUS
					...
					for (int i = 0; i < authors.length; i++) {
						System.out
								.printf("Author #%d : ID=%s, Name=%s, Nationality=%s \n",
										i, authors[i].id, authors[i].name,
										authors[i].nationality);
					}
				}
	...
	public static void addBook(Connection conn, Book boo, Author auth) {
		try {
			...
			String sql = String.format(
					"CALL InsertBook('%s', '%s', '%s', '%s', '%s')", boo.isbn,
					boo.title, boo.pubyear, auth.name, auth.nationality);
	...
	public static void addBorrower(Connection conn, Borrower borr) {
		try {
			Statement st = conn.createStatement();
			String sql = String.format(
					"CALL InsertBorrower('%s', '%s', '%s', '%s')", borr.ssn,borr.name, borr.address, borr.phone);
			ResultSet rs = st.executeQuery(sql);
			rs.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static Author[] getAllAuthors(Connection conn) {
		...
		try {
			...
			rs.beforeFirst();
			while (rs.next())
				authors[i++] = new Author(rs.getInt("authorid"),
						rs.getString("authorname"), rs.getString("authornat"));
	...
	public static Book[] getAllBooks(Connection conn, Author auth) {
		...
		try {
			...
			rs.beforeFirst();
			while (rs.next())
				books[i++] = new Book(rs.getInt("id"), rs.getString("isbn"),
						rs.getString("title"), rs.getString("pubyear"));
	...
	// get AllBooksISBN method used in Q3
	public static Book[] getAllBooksISBN(Connection conn, Book book) {
		Book[] books;

		try {
			Statement st = conn.createStatement();
			ResultSet rs = st.executeQuery("CALL GetAllBooksISBN(\"" + book.isbn	+ "\")");
		...
		public static void lendBook(Connection conn, Borrower borrin) {
			try {
				Statement st = conn.createStatement();
				String sql = String.format(
						"CALL InsertLoan('%s', '%d', '%d','%s')", borrin.ssn,
						borrin.titleno, borrin.copyno, borrin.date);
				ResultSet rs = st.executeQuery(sql);
				rs.close();
			} catch (SQLException e) {
				e.printStackTrace();
